...
position is used. So, the epsilon rate is 01001, or 9 in decimal. Multiplying the gamma rate (22) by the epsilon rate (9)
...
class Day3(object):
    def __init__(self, input_file_path: Path):
        with open(input_file_path, "r") as f:
            self.report = [list(line.strip()) for line in f.readlines()]

        self.report = np.array(self.report).astype(int)

    def part_1(self):
        rows, cols = self.report.shape
        gamma = []
        for col in range(cols):
            gamma.append("1" if self.report[:, col].sum() > rows // 2 else "0")
    ...
    def part_2(self):
        rows = self.report.shape[0]

        # Oxygen generator rating
        bit_position = 0
        oxygen_generator_rating = self.report.tolist()
        while rows > 1:
            most_common = 1 if sum([row[bit_position] for row in oxygen_generator_rating]) >= math.ceil(rows / 2) else 0
            oxygen_generator_rating = [row for row in oxygen_generator_rating if row[bit_position] == most_common]

            rows = len(oxygen_generator_rating)
            bit_position += 1

        oxygen_generator_rating = int("".join([str(o) for o in oxygen_generator_rating[0]]), base=2)

        # CO2 scrubber rating
        rows = self.report.shape[0]

        bit_position = 0
        co2_scrubber_rating = self.report.tolist()
        while rows > 1:
            least_common = 1 if sum([row[bit_position] for row in co2_scrubber_rating]) < math.ceil(rows / 2) else 0
            co2_scrubber_rating = [row for row in co2_scrubber_rating if row[bit_position] == least_common]

            rows = len(co2_scrubber_rating)
            bit_position += 1

        co2_scrubber_rating = int("".join([str(o) for o in co2_scrubber_rating[0]]), base=2)
