...
Instruction = namedtuple("Direction", ["direction", "step"])
...
class Day2(object):
    def __init__(self, input_file_path: Path):
        ...
        with open(input_file_path, "r") as f:
            self.instructions = [Instruction(line.split(" ")[0], int(line.split(" ")[1])) for line in f.readlines()]

    def part_1(self):
        horizontal_pos = depth = 0
        for instruction in self.instructions:
            if instruction.direction == "forward":
                horizontal_pos += instruction.step
            elif instruction.direction == "down":
                depth += instruction.step
    ...
    def part_2(self):
        ...
        for instruction in self.instructions:
            if instruction.direction == "forward":
                horizontal_pos += instruction.step
                depth += aim * instruction.step
            elif instruction.direction == "down":
                aim += instruction.step
            elif instruction.direction == "up":
                aim -= instruction.step
